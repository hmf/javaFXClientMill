 ...
 */
class CandleChartTest extends Application {
    private val INSET:       Double                     = 20
    private var candleChart: CandleChart                = uninitialized
    private var items:       List[CandleChartItem]      = uninitialized
    private var xAxisBottom: Axis                       = uninitialized
    private var yAxisLeft:   Axis                       = uninitialized
    private var grid:        Grid                       = uninitialized
    private var ma5:         Map[ZonedDateTime, Double] = uninitialized
    private var ma10:        Map[ZonedDateTime, Double] = uninitialized
    private var ma20:        Map[ZonedDateTime, Double] = uninitialized
    private var ma60:        Map[ZonedDateTime, Double] = uninitialized
    private var ma5Items:    List[TYChartItem]          = uninitialized
    private var ma10Items:   List[TYChartItem]          = uninitialized
    private var ma20Items:   List[TYChartItem]          = uninitialized
    private var ma60Items:   List[TYChartItem]          = uninitialized
    private var ma5Series:   XYSeries[TYChartItem]      = uninitialized
    private var ma10Series:  XYSeries[TYChartItem]      = uninitialized
    private var ma20Series:  XYSeries[TYChartItem]      = uninitialized
    private var ma60Series:  XYSeries[TYChartItem]      = uninitialized
    private var ma5Chart:    XYChart[TYChartItem]       = uninitialized
    private var ma10Chart:   XYChart[TYChartItem]       = uninitialized
    private var ma20Chart:   XYChart[TYChartItem]       = uninitialized
    private var ma60Chart:   XYChart[TYChartItem]       = uninitialized


    override def init() = {
        items = new ArrayList[CandleChartItem]()
        ma5   = new HashMap[ZonedDateTime, Double]()
        ma10  = new HashMap[ZonedDateTime, Double]()
        ma20  = new HashMap[ZonedDateTime, Double]()
        ma60  = new HashMap[ZonedDateTime, Double]()

        // Prepare data for CandleChart
        prepareData()

        val minItem:CandleChartItem = items.stream().min(Comparator.comparing(_.getLow())).get()
        val maxItem:CandleChartItem = items.stream().max(Comparator.comparing(_.getHigh())).get()
        val minValue: Double        = minItem.getLow()
        val maxValue: Double        = maxItem.getHigh()

        val minTimestamp: Instant = minItem.getTimestamp()
        val maxTimestamp: Instant = maxItem.getTimestamp()
        val days: Long            = Duration.between(maxTimestamp, minTimestamp).toDays()

        val startInstant: Instant = items.stream().min(Comparator.comparing(_.getTimestamp())).get().getTimestamp()
        val endInstant:   Instant = items.stream().max(Comparator.comparing(_.getTimestamp())).get().getTimestamp()
        val start: LocalDateTime  = LocalDateTime.ofInstant(startInstant, ZoneId.systemDefault())
        val end:   LocalDateTime  = LocalDateTime.ofInstant(endInstant, ZoneId.systemDefault())
        ...
        for (item <- items) {
            item.addChartEvtObserver(ChartEvt.ANY, e => {
                val chartItem: ChartItem = e.getSource().asInstanceOf[ChartItem]
                System.out.println(chartItem.getName() + ": " + chartItem.getValue())
            })
        }

        candleChart = CandleChartBuilder.create()
                                        .items(items.asInstanceOf[List[ChartItem]]) // TODO
        ...
        ma5Chart = new XYChart[TYChartItem](new XYPane(ma5Series), yAxisLeft, xAxisBottom)
        ...
        ma60Chart = new XYChart[TYChartItem](new XYPane(ma60Series), yAxisLeft, xAxisBottom)
    ...
    private def createChartItem(DATE: String, LOW: Double, OPEN:Double, CLOSE: Double, HIGH: Double, ma5: java.lang.Double, ma10: java.lang.Double, ma20: java.lang.Double, ma60: java.lang.Double): CandleChartItem = {
        val dateParts: Array[String] = DATE.split("-")
        val MONTH: Int           = Integer.parseInt(dateParts(1))
        val DAY: Int             = Integer.parseInt(dateParts(2))
        val YEAR: Int            = Integer.parseInt(dateParts(0))
        val ZONED_DATE_TIME: ZonedDateTime = ZonedDateTime.of(YEAR, MONTH, DAY, 0, 0, 0, 0, ZoneId.systemDefault())
...
// object CandleChartTest {
//   def main(args: Array[String]) =
//     val app = new CandleChartTest
//     app.launchIt()
// 
// }
...
//     Application.launch(classOf[CandleChartTest], args*)
//     }
// }
